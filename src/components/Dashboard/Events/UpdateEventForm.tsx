...
import { updateEventSchema } from '@/lib/formSchemas'
import { Input } from '@/components/ui/input'
import { Button } from '@/components/ui/button'
import { Checkbox } from '@/components/ui/checkbox'
import { Textarea } from '@/components/ui/textarea'
import { updateEvent } from '@/lib/serverActions/organization'
import { FileUploader } from '@/components/FileUploader'
import { useUploadThing } from '@/lib/uploadthing'
import { cn } from '@/lib/utils'
import { IEvent } from '@/types'

export default function UpdateEventForm({ orgId, event }: { orgId: string; event: IEvent }) {
  console.log(typeof event.max_tickets)
  console.log(event)
  ...
  const form = useForm<z.infer<typeof updateEventSchema>>({
    ...
    defaultValues: {
      name: event.name,
      maxTickets: event.max_tickets.toString(),
      location: event.location,
      description: event.description,
      image: event.cover_image_url,
      status: event.status,
      timeEnd: new Date(event.end_date).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' }),
      timeStart: new Date(event.start_date).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' }),
      dateEnd: new Date(event.end_date).toISOString().split('T')[0],
      dateStart: new Date(event.start_date).toISOString().split('T')[0],
    },
  })
  ...
  async function onSubmit(data: z.infer<typeof updateEventSchema>) {
    ...
    setLoading(true)
    toast.loading('Updating Event...', { id: 'updating-Event' })
    if (logo.length > 0) {
    ...
    toast.dismiss('updating-Event')
    if (!res.success) {
      toast.error('Error updating Event')
      setLoading(false)
      return
    }
    toast.success('Event updat successfully')
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <div className="flex justify-center items-center gap-2">
                  <FormControl>
                    <Input id="maxTickets" type="number" min={1} disabled={noMaxTickets} {...field} />
                  </FormControl>
